...
class ImageReader(object):
  ...
  def __init__(self):
    ...
    self._decode_mp4_data = tf.placeholder(dtype=tf.string)
    self._decode_mp4 = tf.image.decode_mp4(self._decode_mp4_data, channels=1)

  def read_image_dims(self, sess, image_data):
    image = self.decode_mp4(sess, image_data)
    return image.shape[0], image.shape[1]

  def decode_mp4(self, sess, image_data):
    image = sess.run(self._decode_mp4,
                     feed_dict={self._decode_mp4_data: image_data})
...
def _get_filenames_and_classes(dataset_dir):
  ...
  Args:
    dataset_dir: A directory containing a set of subdirectories representing
      class names. Each subdirectory should contain mp4 or JPG encoded images.
...
def _convert_dataset(split_name, filenames, class_names_to_ids, dataset_dir, tfrecord_filename, _NUM_SHARDS):
  ...
  Args:
    ...
    filenames: A list of absolute paths to mp4 or jpg images.
        ...
        with tf.python_io.TFRecordWriter(output_filename) as tfrecord_writer:
          ...
          for i in range(start_ndx, end_ndx):
            ...
            example = image_to_tfexample(
                image_data, b'mp4', height, width, class_id)
